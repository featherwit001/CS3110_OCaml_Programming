...
module Make (Ord: MyOrderType) =  struct
  ...
  let rec find x = function
    ...
    | Node {l; k; d; r; _} -> begin
        ...
        if order = 0 then  d
          ...
          find x (if order > 0 then r else l)
      end
  ...
  let rec bal_height = function
  | Empty -> 0
  | Node {l;r; _} -> begin
    (max (bal_height l) (bal_height r)) + 1
    end

let rec is_bal = function
  | Empty -> true
  | Node {l; r; _} -> begin 
      abs  (bal_height l - bal_height r) <= 1
      && is_bal l
      && is_bal r
    end

let rec is_ordered_maplist = function
  | [] |[_] -> true
  | (kx,_) :: ((ky, _):: _h as h') ->
    if Ord.compare kx ky > 0 then false
    else is_ordered_maplist  h'
let is_ordered m = is_ordered_maplist (binding m)

let is_AVL m =
    is_bal m && is_ordered m
